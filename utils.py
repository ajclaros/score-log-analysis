...
# def create_raster(fish_type, behaviors, colors, duration_behaviors= None):
#     data = pd.read_csv(f"data/{fish_type}/{fish_type}_log.csv")
#     data = data[data['Behavior'].isin(behaviors)]
...
#             if behavior in duration_behaviors:
#                 if row['Behavior'] == behavior:
#                     ax.add_patch(Rectangle((row['Time'], 0.9), width=row['Time2'] - row['Time'], height=0.2, color=colors[i]), label=behavior)
#             else:
#                 if row['Behavior'] == behavior:
#                     print(row['Time'])
#                     ax.eventplot(row['Time'], color=colors[i], label=behavior)
#     plt.legend()
#     plt.show()
